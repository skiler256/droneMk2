...
TEST(SharedStateTest, WriteAndReadBack) {
    ...
    Position pos; pos.ned = {1.0f, 2.0f, -3.0f};
    Velocity vel; vel.ned = {0.5f, 0.0f,  0.0f};
    Attitude att; att.q   = Eigen::Quaternionf::Identity();
    SensorHealth h; h.gps_ok = h.mag_ok = h.lidar_ok = true;
...
TEST(SharedStateTest, IsFreshAfterWrite) {
    SharedStateProvider s;
    Position p; Velocity v; Attitude a; SensorHealth h;
    s.setState(p, v, a, h);
...
TEST(SharedNavOutputTest, ValidCmdIsStored) {
    SharedNavOutput nav;
    VelocityCmd cmd{
        .velocity  = Velocity{Eigen::Vector3f{1.0f, 0.0f, 0.0f}},
        .timestamp = Clock::now()
    };
...
TEST(SharedNavOutputTest, InvalidCmdFallsBackToHold) {
    ...
    VelocityCmd bad_cmd{
        .velocity  = Velocity{Eigen::Vector3f{99.0f, 0.0f, 0.0f}},
        .timestamp = Clock::now()
    };
...
TEST(SharedNavOutputTest, NaNCmdFallsBackToHold) {
    SharedNavOutput nav;
    VelocityCmd nan_cmd{
        .velocity  = Velocity{Eigen::Vector3f{
