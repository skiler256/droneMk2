...
TEST(VelocityCmdTest, NominalIsValid) {
    VelocityCmd cmd{
        .velocity  = Velocity{Eigen::Vector3f{1.0f, 1.0f, 0.5f}},
        .timestamp = Clock::now()
    };
    EXPECT_TRUE(cmd.isValid());
}
...
TEST(VelocityCmdTest, TooFastDescentIsInvalid) {
    VelocityCmd cmd{
        .velocity  = Velocity{Eigen::Vector3f{0.0f, 0.0f, 5.0f}}, // 5 m/s descente > 2
        .timestamp = Clock::now()
    };
    EXPECT_FALSE(cmd.isValid());
}
...
TEST(VelocityCmdTest, TooFastClimbIsInvalid) {
    VelocityCmd cmd{
        .velocity  = Velocity{Eigen::Vector3f{0.0f, 0.0f, -3.0f}}, // -3 m/s montée > 1.5
        .timestamp = Clock::now()
    };
    EXPECT_FALSE(cmd.isValid());
}
...
TEST(VelocityCmdTest, NaNIsInvalid) {
    VelocityCmd cmd{
        .velocity  = Velocity{Eigen::Vector3f{std::numeric_limits<float>::quiet_NaN(), 0.0f, 0.0f}},
        .timestamp = Clock::now()
    };
    EXPECT_FALSE(cmd.isValid());
}
...
TEST(VelocityCmdTest, InfIsInvalid) {
    VelocityCmd cmd{
        .velocity  = Velocity{Eigen::Vector3f{std::numeric_limits<float>::infinity(), 0.0f, 0.0f}},
        .timestamp = Clock::now()
    };
    EXPECT_FALSE(cmd.isValid());
}
...
TEST(NavModeTest, ToStringCoversAllModes) {
    ...
    const NavMode all[] = {
        NavMode::IDLE, NavMode::TAKEOFF, NavMode::WAYPOINT,
...
TEST(SensorHealthTest, NoGpsIsDegraded) {
    SensorHealth h;
    h.gps_ok = false; h.mag_ok = true; h.lidar_ok = true;
    EXPECT_EQ(h.status(), HealthStatus::DEGRADED);
}

TEST(SensorHealthTest, NoGpsNoLidarIsCritical) {
    SensorHealth h;
    h.gps_ok = false; h.mag_ok = true; h.lidar_ok = false;
