...
class SharedSFMemHandler :  public SharedCompMemHandler {
    public:
    explicit SharedSFMemHandler(TYPES::ComponentID id,TYPES::Us timeout,SharedSFMem& SF):
    SharedCompMemHandler(SF.compMem, id, timeout),
    SF_(SF){};
    ...
    void setBlip(int a){
        setData(id_, timeout_, SF_.data.blip, a);
    };
...
return UTILITIES::crc32(SF_.data);
};
