...
struct Position {                   // mètres, NED
    ...
    [[nodiscard]] Meters norm() const noexcept {
        return Meters{ned.norm()};
    }
};
...
struct VelocityCmd {
    ...
    [[nodiscard]] bool isValid() const noexcept {
        ...
        if (std::isnan(vx) || std::isnan(vy) || std::isnan(vz)) return false;
        if (std::isinf(vx) || std::isinf(vy) || std::isinf(vz)) return false;
        const float horiz = std::sqrt(vx*vx + vy*vy);
        if (horiz > MAX_HORIZONTAL_MS)              return false;
        if (vz >  MAX_DESCENT_MS)                  return false; // NED : z>0 = descente
        if (vz < -MAX_CLIMB_MS)                    return false;
        return true;
    }
    ...
    [[nodiscard]] static VelocityCmd hold() noexcept {
        return VelocityCmd{
            .velocity  = Velocity{Eigen::Vector3f::Zero()},
            .timestamp = Clock::now()
        };
    }
};
...
struct SensorHealth {
    ...
    [[nodiscard]] HealthStatus status() const noexcept {
        if (!gps_ok && !lidar_ok) return HealthStatus::CRITICAL;
        if (!gps_ok || !mag_ok)   return HealthStatus::DEGRADED;
...
[[nodiscard]] constexpr std::string_view toString(NavMode m) noexcept {
    switch (m) {
        ...
        case NavMode::TAKEOFF:   return "TAKEOFF";
        case NavMode::WAYPOINT:  return "WAYPOINT";
        case NavMode::HOLD:      return "HOLD";
        case NavMode::LAND:      return "LAND";
        case NavMode::EMERGENCY: return "EMERGENCY";
    }
    return "UNKNOWN";
}
